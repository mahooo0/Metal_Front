...
import { DefaultStartHour, EventGap, EventHeight } from "@/shared/constants";
import { getUkrainianDayName } from "@/shared/lib/ukrainian-locale";
import { Popover, PopoverContent, PopoverTrigger } from "@/shared/ui/popover";
import { useEventVisibility } from "@/shared/use-event-visibility";
import {
  getAllEventsForDay,
  getEventsForDay,
...
export function MonthView({
  ...
  const weekdays = useMemo(() => {
    return Array.from({ length: 7 }).map((_, i) => {
      return getUkrainianDayName(i, true); // true for short names
    });
  }, []);
              ...
              return (
                <div
                  ...
                  <DroppableCell
                    ...
                    className="group-data-today:bg-[#EBFBFF] group-data-today:text-[#1D96F9]">
                    <div className=" mt-1 inline-flex size-6 items-center justify-center rounded-full text-sm">
                      {format(day, "d")}
                    </div>
                    <div
                      ref={isReferenceCell ? contentRef : null}
                      className=" pl-2 min-h-[calc((var(--event-height)+var(--event-gap))*2)] sm:min-h-[calc((var(--event-height)+var(--event-gap))*3)] lg:min-h-[calc((var(--event-height)+var(--event-gap))*4)]">
